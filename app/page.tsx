...
import {
  Mail, FileDown, ArrowRight, CheckCircle2, Users2, Shield
} from "lucide-react";
import {
...
const dict = {
  EN: {
    ...
    infographic: {
      // без NBSP, разрешаем перенос по ТЗ «перенос длинной строки»
...
function getDossierPath(lang: Lang) {
  // Под ваши файлы в /public/files
  const map = {
    EN: "Regulina-T_License_Dossier_EN.docx",
    RU: "Regulina-T_License_Dossier_RU.docx",
    AR: "Regulina-T_License_Dossier_AR.docx"
  } as const;
...
/* ===================== MICRO COMPONENTS (внутри page.tsx) ===================== */
...
function Card({
  ...
  return (
    <div
      className={`mb-6 rounded-2xl border ${clickable ? "transition-shadow hover:shadow-cardHover" : ""} ${className}
                  dark:bg-slate-900/60 dark:border-emerald-900/40`}
      style={{
        borderColor: "#D1FAE5",
        background: "#ECFDF5",
...
function Typewriter({
  ...
  return (
    <span className={className} aria-live="polite">
      {out}
      {!done && <span className="inline-block w-[1px] h-[1em] bg-current ms-1 align-[-0.15em] animate-pulse" aria-hidden="true" />}
    </span>
  );
}
...
function ThemeToggle() {
  ...
  return (
    ...
    >
      {/* простые значки солнце/луна без внешних зависимостей */}
      <svg
        width="16"
        height="16"
        viewBox="0 0 24 24"
...
/* language switch (минималистично, без портала) */
function LanguageSwitcher({
  ...
  const options = (["EN", "RU", "AR"] as Lang[]).filter((l) => l !== lang);
  React.useEffect(() => {
    if (!open) return;
  ...
  function applyLocale(l: Lang) {
    ...
    // Если у вас под-пути /en /ru /ar — можно заменить URL без перезагрузки:
        ...
        >
          {options.map((opt) => (
            ...
            >
              ...
              {/* place for check icon if needed */}
            </button>
          ))}
        </div>
      )}
...
export default function Page() {
  ...
  // init locale from cookie/LS/navigator
  React.useEffect(() => {
  ...
  // Segments (5 equal incl. Transplantation therapy)
  type SegKey = "autoimmune" | "healthyAging" | "infectious" | "oncologyAdjacent" | "transplant";
  const palette: Record<SegKey, string> = {
    autoimmune: "#1E88E5",
    healthyAging: "#0CA678",
    infectious: "#F59E0B",
  ...
  const chartOrder: SegKey[] = ["autoimmune", "healthyAging", "infectious", "oncologyAdjacent", "transplant"];
  const chartData = chartOrder.map((k) => ({ key: k, value: 20 }));
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white text-slate-900 selection:bg-emerald-200/60 dark:bg-slate-950 dark:text-slate-100">
          ...
          <div className={`flex items-start gap-3 min-w-0 ${isRTL ? "flex-row-reverse" : ""}`}>
            ...
            <div className={`flex flex-col leading-none ${isRTL ? "items-end" : "items-start"}`}>
              ...
              <span
                ...
                style={{ borderColor: "#BBF7D0", height: "16px" }}
                aria-hidden="true"
              >
      ...
      <section id="home" className="relative overflow-hidden">
        <div className="mx-auto grid w-full max-w-[1200px] grid-cols-1 items-center gap-10 px-4 py-12 md:grid-cols-2 md:py-20">
          ...
          <div>
            ...
            {/* Paragraph typing */}
            <p className="mt-4 max-w-[88%] md:max-w-[680px] lg:max-w-[720px] text-slate-700 dark:text-slate-300"
          ...
          {/* Right: YouTube video (adaptive 16:9) */}
          <div>
      ...
      <section id="mission" dir={isRTL ? "rtl" : "ltr"} className="px-4">
        <div className="mx-auto w-full max-w-[1200px]">
          <Card className="p-6 md:p-8">
            <div className="mb-3">
              <span className="rt-glass inline-block rounded-full px-4 py-2 text-[20px] md:text-[24px] font-bold text-slate-900 dark:text-slate-100">
                {d.mission.title}
              </span>
            </div>
      ...
      <section id="science" className="px-4">
        <div className="mx-auto grid w-full max-w-[1200px] grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <Card className="p-6 md:p-8 overflow-visible">
            {/* Заголовок: разрешаем перенос + адаптивный кегль; без горизонтального скролла */}
            <h3
                    ...
                    <PieChart>
                      ...
                      >
                        {chartData.map((entry, idx) => (
                          <Cell key={`c-${idx}`} fill={palette[entry.key as SegKey]} />
                        ))}
                      </Pie>
                    </PieChart>
                  </ResponsiveContainer>
                </div>
                {/* Без процентов / тултипов — только визуал */}
              </div>

              {/* Legend */}
              <div className={`grid gap-2 ${isRTL ? "text-right" : "text-left"}`}>
                {[
                  { k: "autoimmune", label: d.segments.autoimmune, color: palette.autoimmune },
                  { k: "healthyAging", label: d.segments.healthyAging, color: palette.healthyAging },
                  { k: "infectious", label: d.segments.infectious, color: palette.infectious },
                  { k: "oncologyAdjacent", label: d.segments.oncologyAdjacent, color: palette.oncologyAdjacent },
                  { k: "transplant", label: d.segments.transplantationTherapy, color: palette.transplant }
                ].map((s) => (
                  <button
                    key={s.k}
        ...
        <div className="mx-auto w-full max-w-[1200px]">
          <Card className="p-6 md:p-8">
            ...
            <p className="text-slate-700 dark:text-slate-300">
              RGN-T1™ Platform — GMP/GLP, dosing safety margin &gt;1000× below endogenous, average schedule 9 doses/year (~1.8 mg).
            </p>
          </Card>
        </div>
      </section>
        ...
        <div className="mx-auto w-full max-w-[1200px]">
          <Card className="p-6 md:p-8">
            ...
            <div className="space-y-2 text-slate-700 dark:text-slate-300">
              <div>Email: <a className="underline" href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a></div>
              <div>Phone (UK): <a className="underline" href="tel:+447440448317">+44 7440 448317</a></div>
            </div>
          </Card>
        </div>
      </section>
      ...
      {/* keyframes для hero (локально, чтобы обойтись одним файлом) */}
      <style jsx global>{`
        .rt-glass {
