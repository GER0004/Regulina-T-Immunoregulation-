...
function Typewriter({
  ...
  useEffect(() => {
    ...
    const begin = window.setTimeout(() => {
      ...
      const tick = () => {
        ...
        if (i < text.length) t = window.setTimeout(tick, 40);
        else setDone(true);
      };
      t = window.setTimeout(tick, 40);
    }, startDelay);
  ...
  return (
    <span className={className}>
      {out}
      {!done && <span className="inline-block w-2 h-5 align-baseline bg-emerald-500 ml-1 animate-pulse" />}
    </span>
  );
}
    ...
    <main className="min-h-screen text-slate-900">
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <TypewriterOnView
            key={lang} // перезапуск анимации при смене языка
      ...
      <section className="mx-auto max-w-6xl px-4" id="mission">
        ...
        <Card className="mt-4">
          <ul className={`grid gap-3 ${isRTL ? "text-right" : ""}`}>
            {d.mission
              .bullets(fmtCompact(lang))
          ...
          >
            <RevealOnView delay={50}>
              <div
                className="rounded-2xl px-4 py-3 text-sm font-semibold bg-white/70 border-2"
                style={{ borderColor: brand.pillBorder }}
              >
      ...
      <section className="mx-auto max-w-6xl px-4 mt-10 grid gap-6 md:grid-cols-2" id="science">
        ...
        {/* Инфографика — ПРАВКА: безопасные паддинги, overflow-hidden, адаптивный контейнер */}
        <div>
          <SectionTitle as="h3">
          ...
          <Card className="mt-4 overflow-hidden">
            <div className="grid gap-5">
              ...
              {/* Легенда — перенос строк, кликабельная зона >= 32px */}
      ...
      {/* ===== Platform (оставил без изменений по твоей просьбе) ===== */}
      <section className="mx-auto max-w-6xl px-4 mt-10" id="platform">
        <SectionTitle as="h3">{d.lbl.platform}</SectionTitle>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4">
          <Feature icon={<Shield className="text-emerald-600" size={18} />} title="GMP/GLP" desc="Quality by design, audit-ready." />
          <Feature icon={<Users2 className="text-emerald-600" size={18} />} title="T-cell focus" desc="Repertoire renewal." />
          <Feature icon={<FlaskConical className="text-emerald-600" size={18} />} title="Bioreactors" desc="Scalable upstream." />
          <Feature icon={<CheckCircle2 className="text-emerald-600" size={18} />} title="Access" desc="Global health impact." />
        </div>
      </section>

      {/* ===== Partnership / Contacts / Footer (сокращённо, без изменений логики) ===== */}
      ...
      <section className="mx-auto max-w-6xl px-4 mt-10" id="contacts">
        ...
        <Card className="mt-4">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <a href={buildContactMailto(lang)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 text-white px-4 py-2">
              <Mail size={16} /> {d.lbl.ctaContact}
            </a>
            <a href={getDossierPath(lang)} className="inline-flex items-center gap-2 rounded-xl border border-emerald-200 bg-white px-4 py-2">
